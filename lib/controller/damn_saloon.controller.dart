...
class DamnSaloonController extends GetxController
    implements SaloonControllerInterface {
  var currentIndex = 0.obs;
  RxString selectedSize = 'Small'.obs; // Fixed typo
  RxString selectedColor = 'Blue'.obs; // Fixed typo
  RxString selectedFullness = ''.obs; // Fixed typo
  @override
  ...
  Future<void> fetchProducts({bool isLoadMore = false}) async {
    ...
    try {
      ...
      final result = await ApiServices.fetchDamnSaloonProducts(
  ...
  Stream<List<SalonService>> getSalonServices() {
    return FirebaseFirestore.instance
        ...
        .where('type', isEqualTo: 'Saloon') // Optional: filter
        .snapshots()
        .map(
          (snapshot) =>
              snapshot.docs
